...
def compose_original_cutout(
    ...
    Args:
        ...
        dialogue_xfade: 相邻对话段之间 0.32s 交叉溶解（dialogue_xfade 配置）；
                    fixed_positions（original_sprite）恒开无需传 True。
    ...
    # --- 对话段交叉溶解：original_sprite（fixed_positions）恒开；其余模式
    # 经 dialogue_xfade 配置开启。此时 segments 下标与 timeline 对齐（占位
    # 已填满），先把连续 dialogue run 合并成叠化块，再走通用 None 过滤 +
    # concat。合并失败自动回退硬切。
    if fixed_positions or dialogue_xfade:
