...
def _validate_script(script: dict, num_lines: int,
                     quest: bool = False,
                     story: bool = False) -> tuple[bool, str]:
    ...
    for i, line in enumerate(dialogue):
        ...
        if not quest and not story and not line.get("phonetic", "").strip():
    ...
    if story:
        if not script.get("listening_question_en", "").strip():
            return False, "Story script has empty 'listening_question_en'"
        allowed = ("char_a", "char_b", "char_c", "char_d", "char_e")
        try:
            from story.quality_gate_story import PHASES as _STORY_PHASES, STORY_KINDS as _KINDS
        except ImportError:
            from quality_gate_story import PHASES as _STORY_PHASES, STORY_KINDS as _KINDS
        kind = str(script.get("story_kind", "plot")).strip().lower()
        if kind not in _KINDS:
            return False, f"Story script has unknown story_kind '{kind}'"
        order = {ph: i for i, ph in enumerate(_STORY_PHASES[kind])}
        phases = [line.get("phase", "") for line in dialogue]
        if any(p not in order for p in phases):
            return False, f"Story dialogue has unknown phase (kind={kind})"
        seq = [order[p] for p in phases]
        if any(b < a for a, b in zip(seq, seq[1:])):
            return False, f"Story dialogue phases are out of order (kind={kind})"
        for i, line in enumerate(dialogue):
            if line.get("speaker", "") not in allowed:
                return False, f"Dialogue line {i} speaker must be one of {allowed}, got '{line.get('speaker', '')}'"
        if str(script.get("welcome_en", "")).strip() or str(script.get("outro", "")).strip():
            return False, "Story script must have empty welcome_en/outro (cold open)"
    elif quest:
        if not script.get("listening_question_en", "").strip():
...
def _story_present_chars(script: dict) -> list[str]:
    """story 模式：本集实际出场的角色键（图集只为出场角色生成）。"""
    keys = [k for k in ("char_a", "char_b", "char_c", "char_d")
            if str(script.get(f"{k}_description", "")).strip()]
    if script.get("has_guest") and str(script.get("char_e_description", "")).strip():
        keys.append("char_e")
...
def _quick_test_fill_placeholders(args, script, img_dir: Path) -> int:
    ...
    elif structure in ("quest", "original_cutout"):
    ...
    if args.resume:
        ...
        if checkpoint:
            ...
            if cp_struct and struct_changed:
                ...
                print(f"  [Resume] Animation changed ({cp_anim} → {args.animation}) — 素材结构不同（图集 vs 正面像），starting fresh.")
                checkpoint = {}
            else:
