...
def _load_used_listening_summaries(lessons_dir: str = None,
    ...
    """Load summaries of previously generated lessons for anti-duplicate.

    Scans a lessons/ directory for JSON files with lesson_type={lesson_type}
    ("listening" or "shorts"). If lessons_dir is None or doesn't exist,
    returns empty list.
    """
    if not lessons_dir:
        return []
...
def _build_shorts_prompt(topic: str, cefr: str, used_dialogues: list[str] = None,
                         num_lines: int = 10) -> str:
    """Build prompt for Shorts culture Q&A script (vertical short video).

    Two friends casually discuss one interesting culture/language knowledge
    point (why Americans do X, what a phrase really means, etc.).
    """
    used_hint = ""
    if used_dialogues:
        used_hint = f"""
IMPORTANT — AVOID DUPLICATES: The following shorts scenarios have already been generated.
Do NOT pick a knowledge point too similar to these:
{chr(10).join(f"  - {d}" for d in used_dialogues[:20])}
"""
    style_prompt = get_active_style_prompt()
    thumb_hint = get_active_thumbnail_hint()
    style_section = f"""
VISUAL STYLE (CRITICAL): The video's art style is: "{style_prompt}".
- EVERY image_prompt and poses entry MUST include this EXACT style descriptor phrase (copy it verbatim).
- Do NOT use any other art style, do NOT mix styles.
"""
    return f"""You are an expert ESL teacher creating a SHORT-FORM video (YouTube Shorts, 45-60 seconds) for overseas Chinese learners.

CORE MISSION: 用一段轻松有趣的好友对话，讲清一个关于美国生活/英语文化的知识点.
{used_hint}
Topic: {topic}
CEFR Level: {cefr}
CEFR Vocabulary Guide:
...
- If the topic is already a question (e.g. "Why Do Baristas Write Names on Cups?"), use it directly as the knowledge question.
- If the topic is a scene name (e.g. "At the Pharmacy"), invent ONE interesting, surprising culture/language question about that scene (e.g. "Why do American pharmacies have drive-thru windows?").
- The answer must be a concrete, real, verifiable cultural fact — NOT a guess. Keep it simple enough to explain in 2-3 sentences.

DIALOGUE STRUCTURE (2 casual friends, natural American English):
- char_a gets curious and asks the question (line 1-2)
- char_b explains the answer with a fun fact or mini-story (middle)
- char_a reacts, asks one follow-up, or shares a related experience
- End with one practical takeaway phrase the viewer can use
- The conversation must sound like REAL friends chatting: contractions, "wait, really?", "no way!", "that makes sense"
- Every line should teach something useful the viewer can repeat

{_build_character_override_prompt(quest=False)}TECHNICAL REQUIREMENTS:
- Exactly 2 speakers (char_a, char_b) — friends, NOT teacher/student
- Exactly {num_lines} dialogue lines (each under 12 words)
- Every dialogue line MUST include:
  - "text": the English sentence
  - "phonetic": IPA phonetic transcription in /slashes/ (proper IPA symbols)
  - "zh": Traditional Chinese (繁體中文) translation
  - "image_prompt": detailed prompt describing the character AND the action, in a VERTICAL 9:16 portrait composition (the video is vertical). MUST include the character's EXACT physical description (same every time for same speaker), the scene, and the action.
  - "video_prompt": same as image_prompt (kept for schema compatibility)
  - "poses": array of exactly 2 pose descriptions for stop-motion animation (each includes the character's full physical description, NO props, NO scene)
- "char_a_description" / "char_b_description": detailed physical descriptions (gender, hair, clothing), used identically in ALL prompts
- "char_a_gender" / "char_b_gender": "male" or "female"
- "char_a_role" / "char_b_role": e.g. "curious friend" / "local friend"
- "title": the knowledge QUESTION in caps (e.g. "WHY DO BARISTAS WRITE NAMES ON CUPS?")
- "title_quote": the most catchy dialogue line, VERBATIM, under 10 words
- "youtube_title_en": the question itself + "| Easy English #Shorts". Example: "Why Do Baristas Write Names on Cups? ☕ Easy English #Shorts"
- "youtube_title": 繁體中文高CTR短标题，【】开头，带 emoji，结尾带 ｜+ English topic。Example: "【英文小知識】☕ 為什麼咖啡師要在杯子上寫名字？美國生活冷知識｜Shorts"
- "youtube_description" / "youtube_description_en": short description with the question, the answer summary, 3 hashtags (#Shorts #LearnEnglish #EasyEnglish). ALL Chinese in Traditional Chinese.
- "youtube_tags": 10-15 SEO tags (include #Shorts related tags)
- "scene": the English scene name
- "thumbnail_prompt": a vertical 9:16 thumbnail prompt in the style hint below
- "thumbnail_expression" / "thumbnail_action" / "thumbnail_subtitle" / "thumbnail_icons": same semantics as long videos (thumbnail_subtitle e.g. "一分鐘小知識")
- "scene_zh": 繁中場景描述
- "title_zh": 繁中問題短標題 (max 8 characters)
- "story_hook": the knowledge question itself (spoken-hook text, one sentence)
- "intro_zh": 繁中版问题
- "outro": a SHORT friendly CTA (under 15 words, e.g. "Follow for more easy English every day!")
- "outro_zh": 繁中 CTA
- "practice_intro_en" / "practice_intro_zh": unused in shorts, set to ""
- ALL Chinese text MUST be in Traditional Chinese (繁體中文)

{style_section}CONSISTENCY RULES (CRITICAL):
- Gender: char_a_gender/char_b_gender MUST match the description text.
- Appearance: each speaker's description MUST be IDENTICAL across ALL their image_prompt/poses entries.
- Scene: MUST be consistent throughout ALL lines.
- Speaker field: MUST use "char_a" or "char_b".
...
def generate_shorts_script(topic: str, cefr: str = "A2",
                           lessons_dir: str = None,
                           num_lines: int = 10) -> dict:
    """Generate a Shorts culture Q&A script (vertical short video).

    Same retry/repair machinery as generate_listening_script, but with
    lesson_type="shorts" and a knowledge-Q&A dialogue prompt.
    """
    used_summaries = _load_used_listening_summaries(lessons_dir, lesson_type="shorts")
    prompt = _build_shorts_prompt(topic, cefr, used_dialogues=used_summaries,
                                  num_lines=num_lines)
    ...
    for attempt in range(3):
        try:
            content = _chat(
                [
                    {"role": "system", "content": "You are an expert ESL teacher creating short-form English learning videos for overseas Chinese learners. Output valid JSON only — no markdown, no explanations."},
                    {"role": "user", "content": prompt},
                ],
                temperature=0.8 if attempt == 0 else 0.7,
                max_tokens=8192,
        ...
        raise RuntimeError(f"LLM shorts script generation failed after 3 retries: {last_error}")

    script["lesson_type"] = "shorts"

    # Ensure all required fields exist (mirror listening defaults)
    script.setdefault("story_hook", "")
    script.setdefault("intro_zh", "")
    script.setdefault("outro", "Follow for more easy English every day!")
    script.setdefault("outro_zh", "")
    script.setdefault("title", "")
    script.setdefault("title_quote", "")
    script["cefr"] = script.get("cefr") or cefr
    script.setdefault("title_zh", script.get("intro_zh", ""))
    script.setdefault("practice_intro_en", "")
    script.setdefault("practice_intro_zh", "")
    script.setdefault("char_a_description", "")
    script.setdefault("char_b_description", "")
    script.setdefault("char_a_gender", "male")
    script.setdefault("char_b_gender", "female")
    script.setdefault("char_a_role", "curious friend")
    script.setdefault("char_b_role", "local friend")
    script.setdefault("youtube_title", "")
    script.setdefault("youtube_title_en", "")
    script.setdefault("youtube_description", "")
    script.setdefault("youtube_description_en", "")
    ...
    script.setdefault("thumbnail_prompt", "")
    script.setdefault("scene", "")
    script.setdefault("thumbnail_expression", "surprised and curious")
    script.setdefault("thumbnail_action", "asking a question with raised hands")
    script.setdefault("thumbnail_subtitle", "一分鐘小知識")
    script.setdefault("thumbnail_icons", [])
