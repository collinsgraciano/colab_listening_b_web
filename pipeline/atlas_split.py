...
def _cell_alpha_bbox(cell):
    """单元格内容 bbox（相对坐标，alpha ≥ _CA_ALPHA_MIN）；空内容返回 None。"""
    if cell.mode != "RGBA":
        return None
    a = np.asarray(cell.getchannel("A"))
    ys, xs = np.where(a >= _CA_ALPHA_MIN)
...
def split_atlas_sequence(atlas_path, cols, rows, out_paths, log_prefix="[SpriteSeq]"):
    """动作序列图集切分（游戏式序列帧专用）。

    与 split_atlas 的关键差异：单元格不做逐格 alpha bbox 收紧/灰边修剪——
    逐格独立裁剪会让每帧的人物大小/位置基准不同，按序列播放时人物抖动。
    透明图集改为全格 union bbox 统一裁剪（所有帧同一相对矩形），
    白底图集保留原格（渲染期统一 remove_bg）。
    边界检测路径与 split_atlas 一致（实线沟 → 内容感知 → 等分回退）。
    """
    atlas = _normalize(Image.open(atlas_path))
    x_bounds, y_bounds, found, notes = detect_grid_boundaries(atlas, cols, rows)
    for note in notes:
    ...
    raw = [atlas.crop(box) for box in cells]
    if _is_transparent_atlas(atlas):
        boxes = [_cell_alpha_bbox(c) for c in raw]
        ...
        if valid:
            ...
            raw = [c.crop((x0, y0, min(c.width, x1), min(c.height, y1)))
                   for c in raw]
            print(f"  {log_prefix} union bbox ({x0},{y0},{x1},{y1}) "
                  f"-> {len(raw)} frames aligned")
    sizes = []
    for idx, cell in enumerate(raw):
        cell.save(out_paths[idx])
        sizes.append(cell.size)
    return sizes
