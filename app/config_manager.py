...
PARAM_SPEC = {
    ...
    "animation": {"default": "stop_motion", "type": "select", "group": "content",
                  ...
                  "help": "仅 Quest 模式可选动画类型。sprite_sequence="
                          "游戏角色式动作序列帧（talking/idle/gesture/wave 4 动作循环，"
                          "AI 视频抽帧路线生成）。旧配置/旧运行里的 none/landing "
                          "自动按 stop_motion 处理（三值渲染等效）。"
                          "original_static 动画强制为 none（不显示此项）；original_cutout 仅定格动画，序列帧请用 Original Sprite 模式"},
    ...
    "character_zh_voices": {"default": "", "type": "text", "group": "content",
                            "modes": ["original", "original_static", "original_cutout", "original_sprite"],
    ...
    "host_character": {"default": "", "type": "select", "group": "content",
                       "modes": ["original_cutout", "original_sprite"],
                       "label": "主持人形象绑定",
    ...
    "dialogue_xfade": {"default": False, "type": "checkbox", "group": "video",
                       "modes": ["original", "original_static", "original_cutout",
                                 "original_cutout_sprite", "quest", "quest_sprite"],
                       "label": "对话段叠化过渡",
    ...
    "render_fps": {"default": 8, "type": "number", "group": "video",
                   "modes": ["original_cutout", "quest", "original_sprite", "quest_sprite"],
                   "label": "渲染帧率 (定格动画)"},
...
def build_cli_args(config: dict[str, Any], resume: bool = False) -> list[str]:
    ...
    if config.get("dialogue_xfade", False):
        args.append("--dialogue-xfade")
    else:
        args.append("--no-dialogue-xfade")
    if config.get("pad"):
        try:
